              ...
              <div class="table-responsive">
                            ...
                            <div class="form-group">
                               <div class="form-line">
								...
								$sql = $koneksi -> query("select * from rak order by nama_rak");
								while ($data=$sql->fetch_assoc()) {
							...
							if (isset($_POST['simpan'])) {
								...
								$sql2= mysqli_query($koneksi,"SELECT * FROM rak");
								$lalas =mysqli_fetch_array($sql2);
								$gudangs= mysqli_query($koneksi,"SELECT * FROM gudang");
								$dadas =mysqli_fetch_array($gudangs);
							
								if ($dadas['jumlah'] < $lalas['space_rak']) {
									# code...
									$sql = $koneksi->query("insert into barang_masuk (id_transaksi, tanggal, kode_barang, nama_barang, jumlah, satuan, pengirim,id_rak) values('$id_transaksi','$tanggal','$kode_barang','$nama_barang','$jumlah','$satuan','$pengirim','$id_rak')");
									...
									echo "<script>alert('Space Rak sudah penuh')</script>";
								}
